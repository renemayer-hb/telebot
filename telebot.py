#!/usr/bin/python
# -*- coding: utf-8 -*-
import subprocess, re, csv, requests, json, telepot, sys, os, time, datetime, psutil, subprocess, importlib, RPi.GPIO as GPIO
		...
		if msg['text'] == "/bm":
			if id in grant:
				keyboard = InlineKeyboardMarkup(inline_keyboard=[
					...
					[
						InlineKeyboardButton(text=_('btn_addbmtg'), callback_data='/tgbmadd'),
						InlineKeyboardButton(text=_('btn_delbmtg'), callback_data='/tgbmdel')
					],
					[
						InlineKeyboardButton(text=_('btn_bmlink'), callback_data='/bmlink'),
						InlineKeyboardButton(text=_('btn_bmunlink'), callback_data='/bmunlink')
					],
					[
...
#				],
		...
		if id in grant:
			...
			time.sleep(2)
			os.system("ifconfig | egrep '([0-9]{1,3}\.){3}[0-9]{1,3}' > " + botpath + "/internalip.txt")
			bot.sendMessage(chat_id, readfile(botpath + "/internalip.txt").replace("          ","").replace("  ",""))
		else:
			msgfuncgrantfehler(msg,chat_id)
